...
struct sort_pair {
    bool operator()(const pair<int, double> &left,
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        ans.push_back({i, tempo});
    }

    int p = 1;

    sort(ans.begin(), ans.end(), sort_pair());

    for (int i = 0; i < 3; i++) {
        cout << setprecision(2) << fixed << p << "o. lugar: competidor "
             << ans[i].first << " - " << ans[i].second << " segundos" << endl;
        p++;
    };
